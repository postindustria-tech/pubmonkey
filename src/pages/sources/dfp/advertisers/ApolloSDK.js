import AbstractAdvertiser from '../../../sources/AbstractAdvertiser'

export class ApolloSDK extends AbstractAdvertiser {

    static advertiser = "apolloSDK";

    customTargetingKey = "hb_pb";

    CREATIVE_FORMATS = {
        "": "All",
        "300x600": "300 x 600",
        "300x50": "300 x 50",
        "320x50": "320 x 50",
        "300x250": "300 x 250",
        "728x90": "728 x 90",
        "970x250": "970 x 250",
        "160x600": "160 x 600",
        "300x100": "300 x 100",
        "970x90": "970 x 90",
    };

    getDefaultSize(params){
        return [1, 1];
    }

    getCreativeHtmlData(params) {
        return '<script type=\"text/javascript\" src = "https://media.admob.com/api/v1/google_mobile_app_ads.js"></script>\n' +
               '<script type=\"text/javascript\">admob.events.dispatchAppEvent(\"OpenXApolloAppEvent\",\"%%PATTERN:bidid%%\");</script>'
    }
}